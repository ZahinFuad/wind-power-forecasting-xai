...
def main():
    ...
    model.save(MODEL_DIR / "gru_baseline_model.h5")
    np.save(DATA_DIR / "y_pred_gru.npy", y_pred)
    np.save(DATA_DIR / "y_test_gru.npy", y_test)
    pd.DataFrame(history.history).to_csv(METRICS_DIR / "training_history_gru.csv", index=False)
    with open(METRICS_DIR / "model_metrics_gru.txt", "w") as f:
        f.write(f"MAE: {mae:.4f}\nRMSE: {rmse:.4f}\n")

    plt.figure(figsize=(12, 5))
    plt.plot(y_test[:200], label="True")
    plt.plot(y_pred[:200], label="Predicted")
    plt.title("GRU Baseline - Wind Power Forecasting - First 200 Samples")
    plt.xlabel("Time Index")
    plt.ylabel("Power Output")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(FIGURE_DIR / "gru_predictions_first_200_samples.png", dpi=300)
    plt.close()
